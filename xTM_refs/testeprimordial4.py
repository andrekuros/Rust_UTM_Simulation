...
OUTPUT_DIR = os.path.join(SCRIPT_DIR, "Resultados_Cenario4A")

# --- CONFIGURAÇÕES DO CENÁRIO 4A (xTM OTIMIZADO + DAA) ---
SCENARIO_ID = 4
SIMULATION_HOURS = 8
MAX_ATRASO_MEDIO_MINUTOS = 5.0  

# A grande sacada: xTM permite tubos muito mais finos (22m) porque o DAA garante a segurança
XTM_SEPARATION_H = 22.0  
XTM_SEPARATION_V = 12.0 

# DAA calibrado para intervir suavemente logo antes do MACproxy
DAA_RADIUS_H = 25.0      
DAA_RADIUS_V = 12.0   
...
class DroneHibrido:
    ...
    def update_hibrido(self, dt, xtm, grid_daa, metrics, tick):
        ...
        if self.state == "FLYING":
            if self.trajectory:
                # O drone puxa a coordenada exata que o xTM mandou ele estar
                _, self.lat, self.lon, self.alt, self.state_display = self.trajectory.pop(0)
                
                # --- DAA TÁTICO ---
                # Se alguém se aproximar mais que o permitido, registramos a evasão
                ...
                # Sendo um cenário determinístico sem ruído, a evasão é apenas o DAA corrigindo as bordas do tubo 4D
                if threat_detected:
                    self.distancia_real_missao += 5.0 # Pequena penalidade de manobra
            else:
                self.state = "IDLE"
...
def run_scenario_scale(num_drones):
    print(f"\n[{num_drones:04} Drones] Iniciando Cenário 4A (xTM Otimizado + DAA)...")
    ...
    print(f"[FIM {num_drones:04} DRONES] Concluído em {tempo_execucao:.1f}s | Atraso Médio: {atraso_medio_min:.1f} min | Ineficiência: {ineficiencia_pct:.1f}% | Evasões DAA: {metrics['ocorrencias_unicas']} | MACproxies: {metrics['macproxy_unicos']}")
...
def gerar_relatorios(resultados):
    ...
    caminho_log = os.path.join(OUTPUT_DIR, "log_consolidado_cenario4A.json")
    ...
    plt.plot(x_drones, y_atraso, marker='o', color='green', linewidth=2, markersize=6)
    ...
    plt.title("Saturação Otimizada: Atraso no Solo com Tubos 4D Reduzidos (Cenário 4A)", fontsize=12)
    ...
    plt.savefig(os.path.join(OUTPUT_DIR, "grafico_atraso_cenario4A.png"), dpi=300, bbox_inches='tight')
    plt.close()
    ...
    if len(resultados) > 0:
        ...
        for nome, res in [("Inicio", res_inicio), ("Intermediario", res_meio), ("Saturado", res_fim)]:
            caminho_pb = os.path.join(OUTPUT_DIR, f"playback_cenario4A_{nome}_{res['num_drones']}drones.json")
...
if __name__ == "__main__":
    ...
    print(f"==================================================")
    print(f" MOTOR DE BUSCA - CENÁRIO 4A (xTM OTIMIZADO)")
    print(f"==================================================\n")
